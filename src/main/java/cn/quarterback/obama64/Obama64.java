...
/**
 * 用于对字符进行编码/解码，类似Base64，可将字符编码为限定码表范围内，编码为不易识别的ASCII字符序列，
 * 并且可根据该编码序列进行解码还原。
 * 
 * Obama64可用于如下等场景：
 ...
 *
 * 基本原理：
 * 		a).我们知道，单字节的编码方案(例如ASCII)，其每个字节的值范围是0~127，而多字节编码方案的字符串，字节值范围为0~255。
 ...
 * 		b).与Base64将原串每6位(bit)扩充成一个新字节(byte)不同: encode时，Obama64采取的方式是，将原串各字节都转换为
 * 	mod 64后对应码表的值，同时在原串每3个字节(如果是encodeAscii，则是6个字节)前插入1个字节，使用这个字节低6位(bit)来记录后续3个字节
 * 	(encodeAscii则是6个字节)mod 64的情况(如a中介绍的原理)，并且.
 * 		c).decode时，则通过解码索引表找到编码字符mod 64前的值，然后根据其对应字节上记录的mod 64的情况，加上N*64还原到编码前的字节值.
 * 
 * @author zhong qiu
 */
